...
assign DOUT[7:0] = ~( RW | ~( PHI2 | DELAY_PHI2 )) ? DOR_LATCH[7:0] : 8'hZZ; // Управление выходом шины данных
// Логика
...
module FLAGS(
   ...
   input PHI2,              // Такт PHI2
	...
	output [7:0]FLAG,        // Выход шины данных флагов
	output P_DB,             // Данные флагов на шину DB
	output reg D_OUT,        // Выход флага D
	output     I_OUT,        // Выход флага I	
	output reg C_OUT,        // Выход флага C	
	output nBRTAKEN,         //
	output reg BRFW          // 	
);
// Переменные
...
reg N_OUT, Z_OUT, V_OUT, I_LATCH1;                              // Первая защелка флага
...
wire FLAG_MUX;
assign FLAG_MUX = ( Z_OUT & ~X[121] & ~X[126] )|( C_OUT & X[121] & ~X[126] )|( V_OUT & ~X[121] & X[126] )|( N_OUT & X[121] & X[126] );
// Выход данных флагов
assign FLAG[7:0] = { N_OUT, V_OUT, 1'b1, B_OUT, D_OUT, I_OUT, Z_OUT, C_OUT};
assign P_DB = ~P_DBR;
assign I_OUT = I_LATCH1 | BRK6E;
assign nBRTAKEN = IR5 ^ FLAG_MUX; // Сигнал готовности перехода
// Логика
always @(posedge Clk) begin 
       if (PHI1) begin 
		 // Первичные защелки флагов
		 C_OUT    <= ( DB[0] & ~DB_CR ) | ( ACR & ~ARC_CR   ) | ( IR5   & IR5_C ) | ( ~( IR5_C | ~ARC_CR | ~DB_CR ) & C_LATCH2 );
		 Z_OUT    <= ( DB[1] & DB_P   ) | ( DBZ & ~DBZ_ZR   ) | ( ~( DB_P | ~DBZ_ZR ) & Z_LATCH2 ); 
		 I_LATCH1 <= ( DB[2] & DB_P   ) | ( IR5 & IR5_I     ) | ( ~( IR5_I | DB_P )   & I_LATCH2 );	 
		 D_OUT    <= ( DB[3] & DB_P   ) | ( IR5 & IR5_D     ) | ( ~( IR5_D | DB_P )   & D_LATCH2 ); 
		 V_OUT    <= ( DB[6] & DB_V   ) | ( AVR & AVR_LATCH ) | ( ~( AVR_LATCH | DB_V | Z_V ) & V_LATCH2 ) | VSET;
	    N_OUT    <= ( DB[7] & DB_N   ) | ( ~DB_N & N_LATCH2 ); 
		 // Защелки V флага
       SO_LATCH1 <= SO;
		 SO_LATCH3 <= ~SO_LATCH2;
		 // Защелка BRFW
...
wire b;
assign b =  ~(( nIRQP | I_OUT ) & nDONMI ) & ( X[80] | T0 );
wire nDONMI;
...
//===============================================================================================
// Модуль управления программным счетчиком (PC)
//===============================================================================================
module PC_SETUP(
   // Такты
   input Clk,               // Тактовый сигнал
   ...
   output PCH_ADH,          // Данные PCH на шину ADH
	...
	output ADL_PCL,          // Загрузка данных из шины ADL
	output nADL_PCL,         //	
   output DL_PCH,           // Загрузка данных из DL в PCH	
	output ADH_PCH,          // Загрузка данных из шины ADH	
	output PCH_PCH,          // Режим хранения данных в разрядах счетчика PCH
	output nPCH_PCH          //		
);
...
module ALU_SETUP(
   ...
   input PHI2,              // Такт PHI2
	...
	input nREADYR,           // Сигнал "Процессор не готов" латч PHI1		
	input PGX,               //
	input D_OUT,             // Вход флага D	
	input C_OUT,             // Вход флага C
	input T0,                // Цикл 0
	input T1,                // Цикл 1
	input T6,                // Цикл 6
	input T7,                // Цикл 7
   ...
   output reg ANDS,         // Логическое И     на выход АЛУ
   output reg ORS,          // Логическое ИЛИ   на выход АЛУ
   output reg nACIN,        // Входной перенос АЛУ
   output reg SRS,          // Сдвиг вправо     на выход АЛУ
   output reg SUMS,	       // Сумма А + В      на выход АЛУ
   output reg EORS,         // Исключающее ИЛИ  на выход АЛУ
   output ADD_SB06,         // Биты выхода АЛУ 0-6 на шину SB	
   output reg ADD_SB7,      // Бит  выхода АЛУ   7 на шину SB	
   output ADD_ADL,          // Выход АЛУ на шину ADL
...
always @(posedge Clk) begin 
       ...
       if (PHI2) begin
		 ...
		 ACIN2      <= INC_SB;
		 ACIN3      <= BRX;
		 ACIN4      <= ~( ~X[54] & ~(( X[52] | X[53] ) & ~( ~C_OUT | ~( T0 | T6 ))));	 
