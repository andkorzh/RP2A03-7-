...
Clk,
PHI0,
1'b1,                  // Отключение десятичной коррекции
1'b1,                  // SO
~NMI,
~( INT | IRQ_EXT ),    // nIRQ
~RES,
...
assign ST1  = SR[0] ? {DEC,BS[10:1]} : BS;                //Первая стадия шифтера
assign ST2  = SR[1] ? {DEC,DEC,ST1[10:2]} : ST1;          //Вторая стадия шифтера
assign   S  = SR[2] ? {DEC,DEC,DEC,DEC,ST2[10:4]} : ST2;  //Третья стадия шифтера
// Сумматор
wire [10:0]SUM;
...
assign NPLAZ    = ~( | FQSOUT[10:0] );                   // NOR от всех выходов LFSR (защита от опустошения LFSR)
assign NPLALS   =  ( | FQSOUT[9:0] | ~FQSOUT[10] );      // OR от всех выходов LFSR c ~10
...
assign RMODE_MUX = RMODE ? RSOUT[8] : RSOUT[13];         // Мультиплексор режима работы Рандом ЛФСР (периодический непериодический шум)
wire [10:0]FQSOUT;
wire [14:0]RSOUT;
//NOISE LFSR
//                        Clk,           SIN,            STEP,  NEXT,   LOAD,    DATA,      SOUT
...
output [14:0]DMCA       // Выход адресной шины DMC
);
// Переменные
...
reg [2:0]EN_LATCH;       // Промежуточные латчи очереди предварительной загрузки
...
reg DMC_DMSTEP_LATCH;    // Промежуточный латч флага STEP
...
//Управление SAMPLE BIT COUNTER, ADDRESS COUNTER, SAMPLE LENGTH DOWN COUNTER, OUTPUT COUNTER
...
output RnW,             // Внешний пин Чтение/Запись
output SPR_PPU          // Контроль выхода спрайтового буффера
);
// Переменные
...
//===============================================================================================
// Модуль реверсивного счетчика
//===============================================================================================
module COUNTER(
// Такты
input Clk,       // Тактовый сигнал
input F2,        // Фаза 2 (ACLK1, PHI1, etc)
// Входы
input DIR,       // Направление счета
...
//===============================================================================================
// Модуль сдвигового регистра
//===============================================================================================
module SHIFTREG(
//Такты
input Clk,      // Тактовый сигнал
// Inputs
input SIN,      // Вход сдвигового регистра
input STEP,     // Сдвиг, фаза1
input NEXT,     // Сдвиг, фаза2
input LOAD,     // Загрузка данных в регистр
input DATA,     // Вход данных
// Outputs
output reg QS   // Выход сдвигового регистра
);
// Переменные
reg QS_IN;      // Первая стадия сдвига
// Логика
always @(posedge Clk) begin
  if (LOAD | STEP) QS_IN <= LOAD ? DATA : SIN;
  if (NEXT) QS <= QS_IN;
                      end
// Конец модуля сдвигового регистра
endmodule
